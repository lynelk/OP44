...
describe('prepaymentBreakdown', () => {
  ...
  it('never goes negative on overpayment', () => {
    expect(prepaymentBreakdown(100000, 250000).remaining).toBe(0);
  });
...
describe('repaymentProgressPct', () => {
  ...
  it('treats null outstanding as fully unpaid', () => {
    expect(repaymentProgressPct(120000, null)).toBe(0);
    expect(repaymentProgressPct(120000, undefined)).toBe(0);
  });
});
