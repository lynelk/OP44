...
/**
 * Reusable error / empty state component.
 *
 * @param {Error|string|null} error      - pass an Error or string for error states
 * @param {'network'|'auth'|'server'|'empty'} [preset] - override auto-classification
 * @param {string}  [title]              - override title
 * @param {string}  [body]               - override description text
 * @param {Function} [onRetry]           - shows a "Try again" button when provided
 * @param {string}  [retryLabel]         - button label (default "Try again")
 */
export default function ErrorState({ error, preset, title, body, onRetry, retryLabel = 'Try again' }) {
  ...
  return (
    <div className={`rounded-2xl p-6 flex flex-col items-center text-center gap-3 ${cfg.bg}`}>
      ...
      <div>
        ...
        {error?.message && !title && (
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 font-mono truncate max-w-xs">{error.message}</p>
        )}
      </div>
      {onRetry && (
        <button
