    ...
    <div className="min-h-screen bg-gray-50 pb-10 font-sans">
      <div className="bg-gradient-to-br from-[#0f2a55] to-[#1e4480] text-white px-5 pt-12 pb-6">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          ...
          <div className="bg-white/10 rounded-2xl p-3 relative">
      ...
      <div className="px-4 mt-4 space-y-3">
        ...
        {activeTab === 'loans' && (
          <div className="space-y-2">
            {loading ? (
              <div className="space-y-2">{[1, 2, 3].map(i => <div key={i} className="bg-white rounded-2xl h-24 animate-pulse" />)}</div>
            ) : loans.length === 0 ? (
              ...
              loans.map(loan => (
                <div key={loan.id} className="bg-white rounded-2xl shadow-sm p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-bold text-gray-900">{loan.loan_ref}</p>
                      <p className="text-xs text-gray-500">UGX {loan.amount_requested?.toLocaleString()} · {loan.tenure_months}mo · Band {loan.risk_band}</p>
                    </div>
        ...
        {activeTab === 'profiles' && (
          <div className="space-y-2">
            {profiles.map(p => (
              <div key={p.id} className="bg-white rounded-2xl shadow-sm p-4 flex items-center justify-between">
                <div>
                  <p className="font-bold text-sm capitalize">{p.account_type} · {p.district || 'N/A'}</p>
                  <p className="text-xs text-gray-500">{p.employment_status} · UGX {p.monthly_income?.toLocaleString()}/mo</p>
                  <p className="text-xs text-gray-400 mt-0.5">KYC: {p.kyc_status} · Trust: {p.trust_score || 0}/100</p>
                </div>
                <div className="text-right">
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${p.account_status === 'active' ? 'bg-emerald-100 text-emerald-700' : p.account_status === 'pending_verification' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-white rounded-2xl p-3 shadow-sm text-center">
                <p className="text-xs text-gray-500">Lender Share</p>
            ...
            {revenue.map(r => (
              <div key={r.id} className="bg-white rounded-2xl shadow-sm p-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${r.status === 'distributed' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{r.status}</span>
                </div>
        ...
        {activeTab === 'config' && (
          <div className="bg-white rounded-2xl shadow-sm p-5 space-y-4">
            <p className="font-bold text-gray-900 flex items-center gap-2"><Settings className="w-4 h-4" /> Revenue Split Configuration</p>
            ...
            <div className="border-t pt-4 space-y-3">
              <p className="font-bold text-gray-900 flex items-center gap-2"><Shield className="w-4 h-4" /> Risk & Rate Config</p>
              {[
